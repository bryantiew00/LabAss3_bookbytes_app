...
$userid = $_GET['userid'];
$sqlloadcart = "SELECT * FROM `tbl_carts` INNER JOIN tbl_books ON tbl_carts.book_id = tbl_books.book_id WHERE tbl_carts.buyer_id = '$userid' AND tbl_carts.cart_status = 'New'";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $cart['order_id'] = $row['order_id'];
        $cart['buyer_id'] = $row['buyer_id'];
